...
class TSPLogicalBrain:
    ...
    def apply_2_opt_refinement(tour):
        """Finding verifiable truth via 2-opt."""
        improved = True
...
class HomeostaticSolver:
    ...
    def solve_trial(self, trial_num):
        ...
        # --- Meta-Stagnation Variables ---
        meta_stagnation_counter = 0
        last_reheat_best_distance = float('inf')
        META_STAGNATION_LIMIT = 2
        # ---
        
        iteration = 0
        progress_update_interval = 40000
        ...
        while temperature > self.params['min_temp'] and not self.stop_event.is_set():
            ...
            # --- Homeostatic Regulation (Reheating) ---
            if stagnation_counter >= self.params['max_stagnation']:
                temperature += self.params['initial_temp'] * 0.1 # Reheat
                stagnation_counter = 0
                
                # --- Meta-Stagnation Check ---
                # Check if the *best score* is still the same as the last time we reheated
                if abs(best_tour.distance - last_reheat_best_distance) < 0.001:
                    meta_stagnation_counter += 1
                else:
                    # We found a new best score, so reset the meta-counter
                    meta_stagnation_counter = 0
                    last_reheat_best_distance = best_tour.distance

                if meta_stagnation_counter >= META_STAGNATION_LIMIT:
                    # print(f" ... [TRIAL {trial_num}] META-STAGNATION: Forcing full reset ...")
                    # "Move out": Jump to a completely new random solution
                    current_tour = Tour(random.sample(self.cities, len(self.cities)))
                    meta_stagnation_counter = 0 # Reset the counter
                    temperature = self.params['initial_temp'] # Reset the temperature
                # --- End of Meta-Stagnation Logic ---
...
def parse_tsp_file(filename):
    ...
    try:
        ...
        print("Please make sure 'berlin52.tsp' is in the same directory as the script.")
        return None
    ...
    coord_regex = re.compile(r"^\s*(\d+)\s+([\d\.]+)\s+([\d\.]+)\s*$")
...
def plot_tour(final_tour, title):
    ...
    x_coords.append(final_tour.tour[0].x)
    
    y_coords = [city.y for city in final_tour.tour]
    y_coords.append(final_tour.tour[0].y)
...
if __name__ == "__main__":
    ...
    known_optimum = 7542
    accuracy_goal_percentage = .99 # 99% of optimal
    ...
    optimality_percentage = (known_optimum / best_overall_tour.distance) * 100
    print(f"This result is {optimality_percentage:.2f}% of the proven optimal solution.")
